...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(gm_mpu6050_interface_M) == (NULL)) &&
    !rtmGetStopRequested(gm_mpu6050_interface_M);
  while (runModel) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(gm_mpu6050_interface_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(gm_mpu6050_interface_M, true);
      }

      if (rtmGetStopRequested(gm_mpu6050_interface_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(gm_mpu6050_interface_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(gm_mpu6050_interface_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(gm_mpu6050_interface_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(gm_mpu6050_interface_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(gm_mpu6050_interface_M, true);
    }
  }
