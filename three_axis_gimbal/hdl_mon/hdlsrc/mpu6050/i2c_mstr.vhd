-- -------------------------------------------------------------
-- 
-- File Name: hdl_mon\hdlsrc\mpu6050\i2c_mstr.vhd
-- Created: 2017-11-17 17:53:03
-- 
...
USE work.I2C_MPU6050_IP_pkg.ALL;
...
ARCHITECTURE rtl OF i2c_mstr IS
...
BEGIN
  ...
  i2c_mstr_1_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      ...
      END IF;
    END IF;
  END PROCESS i2c_mstr_1_process;

  i2c_mstr_1_output : PROCESS (is_i2c_mstr, enb_1, rd_wr, mode, bit_cnt, slv_addr_unsigned, reg_addr_unsigned,
  ...
  BEGIN
    ...
    CASE is_i2c_mstr IS
      WHEN IN_Increment_Byte_count1 =>
        mstr_ack_reg_next <= '0';
        --During 'Increment_Byte_count1': '<S5>:2187'
        --Transition: '<S5>:2271'
        is_i2c_mstr_next <= IN_cmd_reg_read_data_n;
        --Entry 'cmd_reg_read_data_n': '<S5>:2175'
        scl_reg_next <= '0';
        --mstr_ack(:) = 0
        ack_reg_next <= '1';
      WHEN IN_ack_addr_m =>
        scl_reg_next <= '0';
        --During 'ack_addr_m': '<S5>:2183'
        IF reg_addr_burstIn = '1' THEN 
          --Transition: '<S5>:2247'
          is_i2c_mstr_next <= IN_ack_addrm1;
          --Entry 'ack_addrm1': '<S5>:2244'
          scl_reg_next <= '1';
          ack_reg_next <= '0';
      ...
      WHEN IN_ack_addr_m_cnf =>
        scl_reg_next <= '0';
        --During 'ack_addr_m_cnf': '<S5>:2176'
        IF ( NOT rd_wr) = '1' THEN 
          --Transition: '<S5>:2198'
          is_i2c_mstr_next <= IN_cmd_reg_data_n;
          --Entry 'cmd_reg_data_n': '<S5>:2188'
          scl_reg_next <= '0';
          ack_reg_next <= '0';
        ...
        END IF;
      WHEN IN_ack_addrm1 =>
        --During 'ack_addrm1': '<S5>:2244'
        --Transition: '<S5>:2203'
        is_i2c_mstr_next <= IN_cmd_reg_addr_n;
        --Entry 'cmd_reg_addr_n': '<S5>:2172'
        scl_reg_next <= '0';
        ack_reg_next <= '0';
      ...
      WHEN IN_ack_data =>
        --During 'ack_data': '<S5>:2174'
        --Transition: '<S5>:2267'
        is_i2c_mstr_next <= IN_ack_end1;
        --Entry 'ack_end1': '<S5>:2301'
        scl_reg_next <= '1';
        sda_reg_next <= '1';
        ack_reg_next <= '1';
      WHEN IN_ack_end =>
        scl_reg_next <= '1';
        --During 'ack_end': '<S5>:2266'
        ...
        ELSE 
          ...
          --Entry 'error_ack': '<S5>:2171'
          scl_reg_next <= '0';
          sda_reg_next <= '1';
          n_ack_reg_next <= '1';
        END IF;
      WHEN IN_ack_end1 =>
        --During 'ack_end1': '<S5>:2301'
        --Transition: '<S5>:2302'
        is_i2c_mstr_next <= IN_ack_end;
        --Entry 'ack_end': '<S5>:2266'
        scl_reg_next <= '1';
        sda_reg_next <= '1';
        ack_reg_next <= '1';
      WHEN IN_ack_recv_wr =>
        scl_reg_next <= '1';
        --During 'ack_recv_wr': '<S5>:2260'
        IF ( NOT sda_in) = '1' THEN 
          --Transition: '<S5>:2223'
          is_i2c_mstr_next <= IN_ack_wr_p;
          --Entry 'ack_wr_p': '<S5>:2264'
          scl_reg_next <= '0';
          ack_reg_next <= '0';
          sda_reg_next <= '0';
          bit_cnt_next <= to_unsigned(16#08#, 8);
      ...
      WHEN IN_ack_recv_wr1 =>
        --During 'ack_recv_wr1': '<S5>:2278'
        --Transition: '<S5>:2298'
        is_i2c_mstr_next <= IN_ack_recv_wr2;
        --Entry 'ack_recv_wr2': '<S5>:2297'
        scl_reg_next <= '1';
        sda_reg_next <= '1';
        ack_reg_next <= '1';
      WHEN IN_ack_recv_wr2 =>
        scl_reg_next <= '1';
        --During 'ack_recv_wr2': '<S5>:2297'
        IF ( NOT sda_in) = '1' THEN 
          --Transition: '<S5>:2206'
          is_i2c_mstr_next <= IN_ack_addr_m_cnf;
          --Entry 'ack_addr_m_cnf': '<S5>:2176'
          scl_reg_next <= '0';
      ...
      WHEN IN_ack_recv_wr3 =>
        --During 'ack_recv_wr3': '<S5>:2303'
        --Transition: '<S5>:2304'
        is_i2c_mstr_next <= IN_ack_recv_wr;
        --Entry 'ack_recv_wr': '<S5>:2260'
        scl_reg_next <= '1';
        sda_reg_next <= '1';
        ack_reg_next <= '1';
        bit_cnt_next <= to_unsigned(16#08#, 8);
      WHEN IN_ack_slv =>
        --During 'ack_slv': '<S5>:2170'
        --Transition: '<S5>:2261'
        is_i2c_mstr_next <= IN_ack_recv_wr3;
        --Entry 'ack_recv_wr3': '<S5>:2303'
        scl_reg_next <= '1';
        sda_reg_next <= '1';
        ack_reg_next <= '1';
      WHEN IN_ack_slv1 =>
        --During 'ack_slv1': '<S5>:2219'
        --Transition: '<S5>:2259'
        is_i2c_mstr_next <= IN_ack_slv_cnf1;
        --Entry 'ack_slv_cnf1': '<S5>:2194'
        scl_reg_next <= '1';
        ack_reg_next <= '1';
        bit_cnt_next <= to_unsigned(16#08#, 8);
      WHEN IN_ack_slv_cnf1 =>
        scl_reg_next <= '1';
        --During 'ack_slv_cnf1': '<S5>:2194'
        IF ( NOT sda_in) = '1' THEN 
          --Transition: '<S5>:2239'
          is_i2c_mstr_next <= IN_ack_wr_p2;
          --Entry 'ack_wr_p2': '<S5>:2274'
          scl_reg_next <= '0';
          ack_reg_next <= '1';
          sda_reg_next <= '1';
        ELSE 
      ...
      WHEN IN_cmd_reg_addr_n =>
        reg_addr_burst_reg_next <= '0';
        --During 'cmd_reg_addr_n': '<S5>:2172'
        --Transition: '<S5>:2227'
        is_i2c_mstr_next <= IN_cmd_reg_addr_p;
        --Entry 'cmd_reg_addr_p': '<S5>:2178'
        scl_reg_next <= '1';
      ...
      WHEN IN_cmd_reg_addr_p =>
        ...
        ELSE 
          ...
          --Entry 'ack_addr_m': '<S5>:2183'
          scl_reg_next <= '0';
          ack_reg_next <= '1';
          sda_reg_next <= '1';
        END IF;
      WHEN IN_cmd_reg_data_n =>
        --During 'cmd_reg_data_n': '<S5>:2188'
        --Transition: '<S5>:2197'
        is_i2c_mstr_next <= IN_cmd_reg_data_p;
        --Entry 'cmd_reg_data_p': '<S5>:2169'
        scl_reg_next <= '1';
      ...
      WHEN IN_cmd_reg_data_p =>
        ...
        ELSE 
          ...
          --Entry 'ack_data': '<S5>:2174'
          scl_reg_next <= '0';
          sda_reg_next <= '1';
          ack_reg_next <= '1';
        END IF;
      WHEN IN_cmd_reg_read_data_n =>
        --During 'cmd_reg_read_data_n': '<S5>:2175'
        --Transition: '<S5>:2237'
        is_i2c_mstr_next <= IN_cmd_reg_read_data_p;
        --Entry 'cmd_reg_read_data_p': '<S5>:2179'
        scl_reg_next <= '1';
        IF sda_in = '1' THEN 
      ...
      WHEN IN_cmd_reg_read_data_p =>
        scl_reg_next <= '1';
        --During 'cmd_reg_read_data_p': '<S5>:2179'
        IF bit_cnt > to_unsigned(16#00#, 8) THEN 
          --Transition: '<S5>:2235'
          is_i2c_mstr_next <= IN_cmd_reg_read_data_n;
          --Entry 'cmd_reg_read_data_n': '<S5>:2175'
      ...
      WHEN IN_cmd_slave_addr_n =>
        --During 'cmd_slave_addr_n': '<S5>:2166'
        --Transition: '<S5>:2252'
        is_i2c_mstr_next <= IN_cmd_slave_addr_p;
        --Entry 'cmd_slave_addr_p': '<S5>:2167'
        scl_reg_next <= '1';
      ...
      WHEN IN_cmd_slave_addr_n1 =>
        --During 'cmd_slave_addr_n1': '<S5>:2214'
        --Transition: '<S5>:2250'
        is_i2c_mstr_next <= IN_cmd_slave_addr_p1;
        --Entry 'cmd_slave_addr_p1': '<S5>:2241'
        scl_reg_next <= '1';
      ...
      WHEN IN_cmd_slave_addr_p =>
        scl_reg_next <= '1';
        --During 'cmd_slave_addr_p': '<S5>:2167'
        IF bit_cnt > to_unsigned(16#00#, 8) THEN 
          --Transition: '<S5>:2220'
          is_i2c_mstr_next <= IN_cmd_slave_addr_n;
          --Entry 'cmd_slave_addr_n': '<S5>:2166'
          scl_reg_next <= '0';
        ...
        ELSE 
          ...
          --Entry 'write_n': '<S5>:2181'
          scl_reg_next <= '0';
          sda_reg_next <= '0';
        END IF;
      WHEN IN_cmd_slave_addr_p1 =>
        scl_reg_next <= '1';
        --During 'cmd_slave_addr_p1': '<S5>:2241'
        IF bit_cnt > to_unsigned(16#00#, 8) THEN 
          --Transition: '<S5>:2201'
          is_i2c_mstr_next <= IN_cmd_slave_addr_n1;
          --Entry 'cmd_slave_addr_n1': '<S5>:2214'
          scl_reg_next <= '0';
        ...
        ELSE 
          ...
          --Entry 'read_n': '<S5>:2228'
          scl_reg_next <= '0';
          sda_reg_next <= '1';
        END IF;
      WHEN IN_delay =>
        --During 'delay': '<S5>:2180'
        --Transition: '<S5>:2293'
        is_i2c_mstr_next <= IN_ready_repeated;
        --Entry 'ready_repeated': '<S5>:2189'
        scl_reg_next <= '1';
        sda_reg_next <= '1';
      WHEN IN_error_ack =>
        n_ack_reg_next <= '1';
        --During 'error_ack': '<S5>:2171'
        --Transition: '<S5>:2196'
        is_i2c_mstr_next <= IN_scl_ext_n;
        --Entry 'scl_ext_n': '<S5>:2163'
        scl_reg_next <= '0';
      ...
      WHEN IN_mastr_ack_data =>
        mstr_ack_reg_next <= '1';
        scl_reg_next <= '0';
        --During 'mastr_ack_data': '<S5>:2168'
        IF mode = '1' THEN 
          --Transition: '<S5>:2224'
          is_i2c_mstr_next <= IN_mastr_ack_data_cnf;
          --Entry 'mastr_ack_data_cnf': '<S5>:2165'
          scl_reg_next <= '1';
          sda_reg_next <= '0';
          mstr_ack_reg_next <= '1';
        ELSE 
          ...
          --Entry 'mastr_ack_data_cnf1': '<S5>:2281'
          scl_reg_next <= '1';
          sda_reg_next <= '1';
          mstr_ack_reg_next <= '1';
        END IF;
      WHEN IN_mastr_ack_data_cnf =>
        --During 'mastr_ack_data_cnf': '<S5>:2165'
        --Transition: '<S5>:2211'
        is_i2c_mstr_next <= IN_Increment_Byte_count1;
        --Entry 'Increment_Byte_count1': '<S5>:2187'
        scl_reg_next <= '0';
        bit_cnt_next <= to_unsigned(16#08#, 8);
        mstr_ack_reg_next <= '0';
      ...
      WHEN IN_mastr_ack_data_cnf1 =>
        --During 'mastr_ack_data_cnf1': '<S5>:2281'
        --Transition: '<S5>:2282'
        is_i2c_mstr_next <= IN_scl_ext_n;
        --Entry 'scl_ext_n': '<S5>:2163'
        scl_reg_next <= '0';
        ...
        mstr_ack_reg_next <= '0';
      WHEN IN_read_n =>
        --During 'read_n': '<S5>:2228'
        --Transition: '<S5>:2202'
        is_i2c_mstr_next <= IN_read_p;
        --Entry 'read_p': '<S5>:2191'
        scl_reg_next <= '1';
        sda_reg_next <= '1';
        bit_cnt_next <= to_unsigned(16#08#, 8);
      WHEN IN_read_p =>
        --During 'read_p': '<S5>:2191'
        --Transition: '<S5>:2204'
        is_i2c_mstr_next <= IN_ack_slv1;
        --Entry 'ack_slv1': '<S5>:2219'
        scl_reg_next <= '0';
        ack_reg_next <= '1';
        sda_reg_next <= '1';
      WHEN IN_ready =>
        --During 'ready': '<S5>:2156'
        --Transition: '<S5>:2290'
        is_i2c_mstr_next <= IN_start_n;
        --Entry 'start_n': '<S5>:2159'
        scl_reg_next <= '1';
        sda_reg_next <= '0';
      ...
      WHEN IN_ready_repeated =>
        --During 'ready_repeated': '<S5>:2189'
        --Transition: '<S5>:2245'
        is_i2c_mstr_next <= IN_ready_repeated_dummy;
        --Entry 'ready_repeated_dummy': '<S5>:2233'
        scl_reg_next <= '1';
        sda_reg_next <= '1';
      WHEN IN_ready_repeated_dummy =>
        --During 'ready_repeated_dummy': '<S5>:2233'
        --Transition: '<S5>:2200'
        is_i2c_mstr_next <= IN_repeat_strt_n;
        --Entry 'repeat_strt_n': '<S5>:2182'
        scl_reg_next <= '1';
        sda_reg_next <= '0';
        bit_cnt_next <= to_unsigned(16#07#, 8);
        busy_reg_next <= '1';
      WHEN IN_repeat_strt_n =>
        busy_reg_next <= '1';
        scl_reg_next <= '1';
        --During 'repeat_strt_n': '<S5>:2182'
        --Transition: '<S5>:2249'
        is_i2c_mstr_next <= IN_repeat_strt_p;
        --Entry 'repeat_strt_p': '<S5>:2184'
        sda_reg_next <= '0';
      WHEN IN_repeat_strt_p =>
        --During 'repeat_strt_p': '<S5>:2184'
        --Transition: '<S5>:2248'
        is_i2c_mstr_next <= IN_cmd_slave_addr_n1;
        --Entry 'cmd_slave_addr_n1': '<S5>:2214'
        scl_reg_next <= '0';
      ...
      WHEN IN_scl_ext_n =>
        mstr_ack_reg_next <= '0';
        --During 'scl_ext_n': '<S5>:2163'
        --Transition: '<S5>:2229'
        is_i2c_mstr_next <= IN_scl_ext_p;
        --Entry 'scl_ext_p': '<S5>:2157'
        scl_reg_next <= '1';
        sda_reg_next <= '0';
        ack_reg_next <= '0';
      WHEN IN_scl_ext_p =>
        scl_reg_next <= '1';
        --During 'scl_ext_p': '<S5>:2157'
        --Transition: '<S5>:2209'
      ...
      WHEN IN_start_n =>
        busy_reg_next <= '1';
        scl_reg_next <= '1';
        --During 'start_n': '<S5>:2159'
        --Transition: '<S5>:2210'
        is_i2c_mstr_next <= IN_start_p;
        --Entry 'start_p': '<S5>:2164'
        sda_reg_next <= '0';
      WHEN IN_start_p =>
        --During 'start_p': '<S5>:2164'
        --Transition: '<S5>:2291'
        is_i2c_mstr_next <= IN_cmd_slave_addr_n;
        --Entry 'cmd_slave_addr_n': '<S5>:2166'
        scl_reg_next <= '0';
      ...
      WHEN IN_stop =>
        ...
        --Entry 'stop_ext': '<S5>:2162'
        scl_reg_next <= '1';
        sda_reg_next <= '1';
        ack_reg_next <= '0';
      WHEN IN_stop_ext =>
        --During 'stop_ext': '<S5>:2162'
        --Transition: '<S5>:2215'
        ...
        temp_data_next <= to_unsigned(16#00#, 8);
      WHEN IN_write_n =>
        --During 'write_n': '<S5>:2181'
        --Transition: '<S5>:2225'
        is_i2c_mstr_next <= IN_write_p;
        --Entry 'write_p': '<S5>:2185'
        scl_reg_next <= '1';
        sda_reg_next <= '0';
      WHEN OTHERS => 
        --During 'write_p': '<S5>:2185'
        --Transition: '<S5>:2218'
        is_i2c_mstr_next <= IN_ack_slv;
        --Entry 'ack_slv': '<S5>:2170'
        scl_reg_next <= '0';
        ack_reg_next <= '1';
        sda_reg_next <= '1';
    END CASE;
  END PROCESS i2c_mstr_1_output;
